...
def printDist(dist, m=8, sort="prob", padding=50):
    c = 0
    s = sorted(dist.items(), key=lambda a: -
               a[1]) if sort == "prob" else sorted(dist.items(), key=lambda a: a[0])
    p = 1
    for e in s:
        ...
        if (c == m-1) and (len(s) - c > 1):
            print("{}: {}%".format(
                "---other---".rjust(padding), format(100*p, '>5.2f')))
            break
...
def updateTuple(NamedTuple, nameOfNamedTuple, changes):
    ...
    updated_NamedTuple = collections.namedtuple(
        nameOfNamedTuple, NamedTuple_asdict.keys())(**NamedTuple_asdict)
